...
namespace HtmlAgilityPack
{
	...
	public class HtmlNode : IXPathNavigable
	{
		...
		public void SetStyleValue(string name, string value)
		{
			...
			HtmlAttribute att = Attributes["style"];
			if (att != null)
			{
				styles = att.Value;
				if (styles != null)
			...
			{
				//existing style empty
				if (!string.IsNullOrEmpty(value))
				{
					//case 1 - single style created
					styles = string.Format(CultureInfo.InvariantCulture, "{0}:{1};", name, value);
				}
				else
				{
					//case 2 - no style created
				}
			}
			else
			{
				int pos0 = -1;
				int pos1 = -1;
				if (styles.StartsWith(string.Format(CultureInfo.InvariantCulture, "{0}:", name), StringComparison.OrdinalIgnoreCase))
				{
					pos0 = 0;
					pos1 = pos0 + name.Length + 1;
				}
				else
				{
					pos0 = styles.IndexOf(string.Format(CultureInfo.InvariantCulture, " {0}:", name), StringComparison.OrdinalIgnoreCase);
					if (pos0 < 0)
					{
						pos0 = styles.IndexOf(string.Format(CultureInfo.InvariantCulture, ";{0}:", name), StringComparison.OrdinalIgnoreCase);
					}
					pos1 = pos0 + name.Length +2;
				}
				if (pos0 < 0)
				{
					//not exist
					if (!string.IsNullOrEmpty(value))
					{
						//case 3 - style appended to the end
						if (!styles.EndsWith(";"))
						{
							styles = string.Format(CultureInfo.InvariantCulture, "{0};", styles);
						}
						styles = string.Format(CultureInfo.InvariantCulture, "{0}{1}:{2};", styles, name, value);
					}
					else
					{
						//case 4 - existing style not changed
					}
				}
				else
				{
					//find ending
					int posEnd = styles.IndexOf(';',pos1);
					if (posEnd < 0)
					{
						//the last
						if (string.IsNullOrEmpty(value))
						{
							//case 5 -- exisitng last style removed
							if (styles[pos0] == ';')
								styles = styles.Substring(0, pos0 + 1);
							else
								styles = styles.Substring(0, pos0);
						}
						else
						{
							//case 6 -- existing last style modified
							styles = string.Format(CultureInfo.InvariantCulture, "{0}{1}", styles.Substring(0, pos1), value);
						}
					}
					else
					{
						...
						if (string.IsNullOrEmpty(value))
						{
							//case 7 - existing non-last style removed
							styles = string.Format(CultureInfo.InvariantCulture, "{0}{1}", styles.Substring(0, pos0), styles.Substring(posEnd + 1));
						}
						else
						{
							//case 8 - existing non-last style modified
							styles = string.Format(CultureInfo.InvariantCulture, "{0}{1}{2}", styles.Substring(0, pos1), value, styles.Substring(posEnd));
						}
					}
				}
			}
			if (att == null)
			{
				if (!string.IsNullOrEmpty(styles))
				{
					Attributes.Append(_ownerdocument.CreateAttribute("style", styles));
				}
			}
			else
			{
