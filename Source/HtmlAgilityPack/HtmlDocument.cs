...
namespace HtmlAgilityPack
{
	...
	public class HtmlDocument : IXPathNavigable
	{
		...
		public static void writeHtmlNode(HtmlNode node, StreamWriter sw, int indent)
		{
			if (string.CompareOrdinal(node.Name, "br") == 0)
			{
				sw.Write("<br>\r\n");
				return;
			}
			if (string.CompareOrdinal(node.Name, "#text") == 0)
			{
				string txt = node.InnerHtml;
				if (txt != null)
					txt = txt.Trim();
				if (string.IsNullOrEmpty(txt) || string.CompareOrdinal(txt, "\n") == 0 || string.CompareOrdinal(txt, "\r\n") == 0)
				{
				}
				else
				{
			...
			if (string.CompareOrdinal(node.Name, "#document") == 0)
			{
				for (int i = 0; i < node.ChildNodes.Count; i++)
				{
					writeHtmlNode(node.ChildNodes[i], sw, indent);
				}
				return;
			}
			else if (string.CompareOrdinal(node.Name, "#comment") == 0)
			{
			...
			StringBuilder line = new StringBuilder(sIndent);
			line.Append("<");
			...
			if (node.Attributes.Count > 0)
			{
				...
				{
					line.Append(" ");
					line.Append(node.Attributes[i].Name);
					line.Append("=\"");
					line.Append(node.Attributes[i].Value);
					line.Append("\"");
				}
			}
			if (string.Compare(node.Name, "script", StringComparison.OrdinalIgnoreCase) == 0)
			{
				sw.Write(line);
				sw.Write(">");
			...
			{
				if (node.ChildNodes.Count == 1 && string.CompareOrdinal(node.ChildNodes[0].Name, "#text") == 0)
				{
					sw.Write(line);
					sw.Write(">");
					sw.Write(node.ChildNodes[0].InnerHtml);
				}
				else
				{
					line.Append(">\r\n");
					...
					for (int i = 0; i < node.ChildNodes.Count; i++)
					{
						writeHtmlNode(node.ChildNodes[i], sw, indent + 1);
					}
					sw.Write(sIndent);
				}
				sw.Write("</");
