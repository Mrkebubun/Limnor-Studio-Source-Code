...
var datePickerController = (function datePickerController() {
    ...
    function datePicker(options) {
        ...
        for (var thing in options) {
            if (thing.search(/callbacks|formElements|formatMasks|toJSONString|parseJSON/) != -1) continue;
        ...
        this.create = function() {
            ...
            if (this.bounds) {
                if (this.bounds.width && this.bounds.height) {
                    this.div.style.width = this.bounds.width;
                    this.div.style.height = this.bounds.height;
                }
            }
            ...
            if (this.bounds) {
                if (this.bounds.width && this.bounds.height) {
                    this.table.style.width = this.bounds.width;
                    this.table.style.height = this.bounds.height;
                }
                if (this.bounds.fontSize) {
                    this.table.style.fontSize = this.bounds.fontSize;
                }
            }
            ...
            if (!this.staticPos) {
                ...
                if (this.bounds.container) {
                	this.bounds.container.appendChild(this.div);
                }
                else {
            ...
            if (this.bounds) {
            	//if (this.bounds.inputTime) {
            	if (typeof tableFoot == 'undefined') {
            		tableFoot = document.createElement('tfoot');
            		this.table.appendChild(tableFoot);
            	}
            	tr = document.createElement('tr');
            	...
            	tableFoot.appendChild(tr);
            	//
            	var td = createTH({ thClassName: "date-picker-statusbar" + dragEnabledCN, colspan: this.showWeeks ? 8 : 7 });
            	...
            	selH.id = "tm_h_" + this.id;
            	for (var i = 0; i < 24; i++) {
            		selH.options[i] = new Option(pad(i), pad(i), true, true);
            	}
            	...
            	selM.id = "tm_m_" + this.id;
            	for (var i = 0; i < 60; i++) {
            		selM.options[i] = new Option(pad(i), pad(i), true, true);
            	}
            	...
            	selS.id = "tm_s_" + this.id;
            	for (var i = 0; i < 60; i++) {
            		selS.options[i] = new Option(pad(i), pad(i), true, true);
            	}
        ...
        this.fade = function() {
            ...
            o.fadeTimer = null;
            if (o.bounds && o.bounds.forStandalone) return;
        ...
        this.trackDrag = function (e) {
        	if (o.bounds && o.bounds.disableMove) return;
            e = e || window.event;
        ...
        this.onmousedown = function (e) {
            ...
            if (origEl.id.search(new RegExp("^" + o.id + "(-prev-year-but|-prev-month-but|-next-month-but|-next-year-but)$")) != -1) {
            	...
            	if (origEl.tagName && origEl.tagName.toLowerCase() == 'th') {
            		if (!o.dragging) {
            			o.dragging = true;
        ...
        this.onclick = function (e) {
        	if (o.disabled) return;
        	if (!o.bounds.forStandalone && (o.opacity != o.opacityTo || o.disabled)) return stopEvent(e);
            ...
            while (el.parentNode) {
                ...
                if (el.tagName && el.tagName.toLowerCase() == "td") {
                    ...
                    var eHrs = document.getElementById('tm_h_' + o.id);
                    if (typeof eHrs != 'undefined') {
                        if (eHrs.selectedIndex >= 0) {
                            hrs = eHrs.selectedIndex;
                        }
                    }
                    var eMins = document.getElementById('tm_m_' + o.id);
                    if (typeof eMins != 'undefined') {
                        if (eMins.selectedIndex >= 0) {
                            mins = eMins.selectedIndex;
                        }
                    }
                    var eSecs = document.getElementById('tm_s_' + o.id);
                    if (typeof eSecs != 'undefined') {
                        if (eSecs.selectedIndex >= 0) {
                            secs = eSecs.selectedIndex;
                        }
                    }

                    var cellDate = el.className.match(/cd-([0-9]{8})/)[1];
                    o.date = new Date(cellDate.substr(0, 4), cellDate.substr(4, 2) - 1, cellDate.substr(6, 2), hrs, mins, secs, 0);
                    o.dateSet = new Date(o.date);
                    o.noFocus = true;
                    o.callback("dateset", { "id": o.id, "date": o.dateSet, "dd": o.dateSet.getDate(), "mm": o.dateSet.getMonth() + 1, "yyyy": o.dateSet.getFullYear(), "hh": o.dateSet.getHours(), "min": o.dateSet.getMinutes(), "sec": o.dateSet.getSeconds() });
        ...
        this.show = function(autoFocus) {
            ...
            for (elemID in this.formElements) {
                var elem0 = document.getElementById(elemID);
                if (!elem0) continue;
                elem = elem0;
        ...
        this.hide = function() {
        	...
        	if (this.datepickerStandaloe) return;
    ...
    datePicker.prototype.createButton = function() {
        ...
        if (this.bounds) {
            but.style.position = this.bounds.position;
            if (this.bounds.inputRight) {
                but.style.top = inp.offsetTop + 'px';
                but.style.left = (inp.offsetLeft + inp.offsetWidth) + 'px';
            }
            else {
                but.style.top = this.bounds.y;
                but.style.left = this.bounds.x;
            }
        }
    ...
    datePicker.prototype.setSelectedTime = function (hh, mm, ss) {
    	if (this.dateSet) {
    	}
    }
    datePicker.prototype.setSelectedDate = function (yyyymmdd, hh, mm, ss) {
        ...
        if (typeof hh != 'undefined' && hh != null) {
        	hrs = parseInt(hh, 10);
        	if (hrs < 0 || hrs >= 24) hrs = 0;
        }
        if (typeof mm != 'undefined' && mm != null) {
        	mins = parseInt(mm, 10);
        	if (mins < 0 || mins >= 60) mins = 0;
        }
        if (typeof ss != 'undefined' && ss != null) {
        	secs = parseInt(ss, 10);
        	if (secs < 0 || secs >= 60) secs = 0;
        }
        dt.setHours(hrs, mins, secs);
        this.dateSet = new Date(dt);
        var eHrs = document.getElementById('tm_h_' + this.id);
        if (eHrs) {
        	eHrs.selectedIndex = hrs;
        }
        var eMins = document.getElementById('tm_m_' + this.id);
        if (eMins) {
        	eMins.selectedIndex = mins;
        }
        var eSecs = document.getElementById('tm_s_' + this.id);
        if (eSecs) {
        	eSecs.selectedIndex = secs;
        }
    ...
    datePicker.prototype.setDateFromInput = function() {
        ...
        for (elemID in this.formElements) {
            ...
            dt = false;
            var date_time = elemVal.split(' ');
            if (date_time.length == 2) {
                elemVal = date_time[0];
                var d_time = date_time[1].split(':');
                if (d_time.length == 3) {
                    hrs = d_time[0]; mins = d_time[1]; secs = d_time[2];
                }
            }
    ...
    datePicker.prototype.returnFormattedDate = function(noFocus) {
        ...
        for (elemID in this.formElements) {
            ...
            if (elem.tagName.toLowerCase() == "input") {
                if (this.bounds && this.bounds.inputTime) {
                    elem.value = fmtDate + ' ' + pad(this.dateSet.getHours()) + ':' + pad(this.dateSet.getMinutes()) + ':' + pad(this.dateSet.getSeconds());
                }
                else {
                    elem.value = fmtDate;
                }
                if (elem.onsetbounddata) {
                    elem.onsetbounddata(elem);
                }
            } else {
    ...
    datePicker.prototype.enableTimeSelects = function (d) {
    	var eHrs = document.getElementById('tm_h_' + this.id);
    	if (eHrs) {
    		eHrs.disabled = d ? false : true;
    	}
    	var eMins = document.getElementById('tm_m_' + this.id);
    	if (eMins) {
    		eMins.disabled = d ? false : true;
    	}
    	var eSecs = document.getElementById('tm_s_' + this.id);
    	if (eSecs) {
    		eSecs.disabled = d ? false : true;
    	}
    };
    ...
    datePicker.prototype.enableDatePicker = function() {
        ...
        this.enableTimeSelects(this.bounds.inputTime);
        this.disabled = false;
    };
    datePicker.prototype.setTimeEnabled = function (enable) {
    	this.bounds.inputTime = enable;
    	this.enableTimeSelects(this.bounds.inputTime && !this.disabled);
    };
    datePicker.prototype.getTimeEnabled = function () {
    	return this.bounds.inputTime;
    };
    datePicker.prototype.adjustButtonPos = function () {
    	var but = document.getElementById("fd-but-" + this.id);
    	if (but) {
    		var inp = document.getElementById(this.id);
    		if (inp) {
    ...
    var cleanUp = function() {
        ...
        for (dp in datePickers) {
            for (fe in datePickers[dp].formElements) {
                if (fe == 'toJSONString') continue;
                if (fe == 'parseJSON') continue;
    ...
    var destroy = function(e) {
        ...
        for (var dp in datePickers) {
            if (typeof (datePickers[dp].destroy) == 'function') {
                datePickers[dp].destroy();
                datePickers[dp] = null;
                delete datePickers[dp];
            }
    ...
    var addDatePicker = function(options) {
        ...
        var button = document.getElementById("fd-but-" + options.id);
        return button;
    };
    ...
    return {
        ...
        createDatePicker: function(options) { return addDatePicker(options); },
        ...
        setSelectedDate: function (inpID, yyyymmdd, hh, mm, ss) { if (!inpID || !(inpID in datePickers)) { return false; }; datePickers[inpID].setSelectedDate(yyyymmdd, hh, mm, ss); },
        setSelectedTime: function (inpID, hh, mm, ss) { if (!inpID || !(inpID in datePickers)) { return false; }; datePickers[inpID].setSelectedTime(hh,mm,ss); },
        ...
        setEnableTime: function (inpID, enable) { if (!inpID || !(inpID in datePickers)) return false; datePickers[inpID].setTimeEnabled(enable); },
        getEnableTime: function (inpID) { if (!inpID || !(inpID in datePickers)) return false; return datePickers[inpID].getTimeEnabled(); },
        adjustButtonPos: function (inpID) { if (!inpID || !(inpID in datePickers)) return false; return datePickers[inpID].adjustButtonPos(); },
        // Set the cursor date
