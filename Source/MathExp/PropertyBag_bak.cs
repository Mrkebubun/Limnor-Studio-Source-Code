...
namespace MathExp
{
	...
	public class PropertyBag : ICustomTypeDescriptor
	{
        ...
        public void SetPropertyValue(string name, object v)
        {
        }
		...
		PropertyDescriptorCollection ICustomTypeDescriptor.GetProperties(Attribute[] attributes)
		{
			...
			foreach(PropertySpec property in properties)
			{
                ...
                {
                    object editor = TypeDescriptor.GetEditor(Type.GetType(property.TypeName), typeof(UITypeEditor));
                    if (editor != null)
                    {
                        attrs.Add(new EditorAttribute(editor.GetType().AssemblyQualifiedName, typeof(UITypeEditor)));
                    }
                }
                ...
                {
                    TypeConverter converter = TypeDescriptor.GetConverter(Type.GetType(property.TypeName));
                    if (converter != null)
                    {
                        attrs.Add(new TypeConverterAttribute(converter.GetType().AssemblyQualifiedName));
                    }
                }
