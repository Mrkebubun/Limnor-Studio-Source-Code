...
JsonDataBinding.DialogueBox = {
	...
	showDialog: function(dlg) {
		...
		function attachWaitFrmEvent() {
			_docWait = _frmWait.contentDocument ? _frmWait.contentDocument : _frmWait.contentWindow.document;
			if (_docWait.readyState == 'complete') {
				...
				setTimeout(attachWaitFrmEvent, 300);
			}
		}
		...
		function attachDlgDocEvent() {
			_docDialog = _frmDialog.contentDocument ? _frmDialog.contentDocument : _frmDialog.contentWindow.document;
			if (_docDialog.readyState == 'complete' && (dlg.url == 'about:blank' || dlg.url == null || _docDialog.URL != 'about:blank')) {
				...
				setTimeout(attachDlgDocEvent, 300);
			}
		}
		...
		//_divDialog
		//  tbl--------------
		//    tr title bar: icon, _tdTitle, _imgOK, imgCancel
		//    tr page:_frmDialog
		//--------------------
		if (!_divDialog) {
			...
			if (dlg.contentsHolder) {
				...
				_divDialog.style.height = 'auto';// '100%';
			...
			JsonDataBinding.AttachEvent(imgCancel, "onclick", onCancel);
			//
			attachWaitFrmEvent();
			//
		}
...
JsonDataBinding.childWindows = function() {
    ...
    _getChildByPageId = function(pid) {
        if (_childObjs) {
            for (var i = 0; i < _childObjs.length; i++) {
                if (_childObjs[i].getPageId() == pid) {
                    return _childObjs[i];
    ...
    _onChildMouseDown = function(pid, e) {
        var api = _getChildByPageId(pid);
        if (api) {
            api.onDocMouseDown(e);
        }
    }
    _onChildMouseMove = function(pid, e) {
        var api = _getChildByPageId(pid);
        if (api) {
            api.onDocMouseMove(e);
        }
    }
    _onChildMouseUp = function(pid, e) {
        var api = _getChildByPageId(pid);
        if (api) {
            api.onDocMouseUp(e);
        }
    ...
    _setProperty = function(name, val) {
        JsonDataBinding.PageValues[name] = val;
    }
    _getServerProperty = function(name) {
        return JsonDataBinding.values[name];
    }
    _execute = function(methodName, args) {
        ...
        if (window.parent && window != window.parent) {
            if (!JsonDataBinding.IsFireFox() && !JsonDataBinding.IsIE()) {
                ...
                function onDocMouseUp(e) {
                    window.parent.document.childManager.onChildMouseUp(document.pageId, e);
                }
...
JsonDataBinding.closePage = function() {
    if (window != window.parent) {
        if (window.parent.document.childManager) {
            window.parent.document.childManager.closeWindow(document.pageId);
        }
    }
}
