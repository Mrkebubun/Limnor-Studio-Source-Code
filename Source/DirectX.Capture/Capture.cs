...
namespace DirectX.Capture
{
	...
	public class Capture
	{
        ...
        /// <summary>
        /// Do we need the capture stream rendered
        /// </summary>
		...
		protected void createGraph()
		{
			...
			if ( (int)graphState < (int)GraphState.Created )
			{
				...
				hr = captureGraphBuilder.FindInterface(
					cat, med, audioDeviceFilter, iid, out o );
				if ( hr != 0 )
					o = null;
		...
		protected void renderGraph()
		{
			...
			if ( wantPreviewRendered && !isPreviewRendered )
			{
				...
				hr = videoWindow.put_Visible(OABool.True);// DsHlp.OATRUE );
				if( hr < 0 ) Marshal.ThrowExceptionForHR( hr );
		...
		protected void derenderGraph()
		{
			...
			if ( videoWindow != null )
			{
				videoWindow.put_Visible(OABool.False);// DsHlp.OAFALSE );
		...
		protected object getStreamConfigSetting( IAMStreamConfig streamConfig, string fieldName)
		{
            ...
            AMMediaType mediaType = null;// new AMMediaType();
